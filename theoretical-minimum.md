...
To create UI software, we need a system, commonly referred to as a UI toolkit, that provides development-time and runtime support. In Web developmemnt, the browser itself is the UI toolkit, providing the underlying infrastructure that handles input (event retrieving and dispatching), output (layout and drawing to screen) and manages state (DOM tree).
