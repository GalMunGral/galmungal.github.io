...
To create UI software, we need a system, commonly referred to as a UI toolkit, to provide development-time and runtime support. In Web developmemnt, the browser itself is the UI toolkit that handles input (event dispatching), output (layout and drawing), etc. behind the scenes.
